...
`r chunk_reveal("cars3", title = "## A title several widths", widths = c(22, 30, 40), display_type = c("code", "func", "output"))`
...
```{r, echo = F}
"Sometimes, it's helpful to slow down and look at exactly one decision.| Flipbooks present code step-by-step and side-by-side with output.| They parse code pipelines.| Reconstruct code into partial builds.| And present those builds with their output.| Both code and output are superimposed on the previous state." ->
  philosophy
```
...
to create a pdf from you may use the pagedown package as follows to use chrome print to pdf:
...
Coming soon, build a previews, using [viztoc](https://github.com/EvaMaeRey/viztoc). Development package!  Breaking changes planned. 
