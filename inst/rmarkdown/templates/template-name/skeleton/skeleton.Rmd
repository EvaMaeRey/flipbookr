...
```markdown
``r "r reveal(chunk_name = \"cars\", reveal_type = \"user\", display_type = \"both\", left_assign = FALSE)"``
``` 
...
- **reveal type** -- *which lines of code should be revealed when*, `reveal_type` defaults to "auto"
...
Notice the regular comments and the special #REVEAL comments, these will be used for a couple of the different "reveal type" modalities.
...
One parameter of flipbooking is the reveal_type.  The default is "auto", in which appropriate revealpoints are determined automatically --- by finding where parentheses are balanced. 
...
`r reveal("cars", reveal_type = "auto")`

---

## reveal_type = "user", with #REVEAL

If the reveal_type is set to "user", the revealpoints are those indicated by the user with the special comment #REVEAL
...
`r reveal("cars", reveal_type = "user")`

---

## reveal_type = "non_seq", with #REVEAL2, #REVEAL3

If the reveal_type is set to "non_seq", the revealpoints are those indicated by the user with the special numeric comment #REVEAL2, #REVEAL3 etc to indicate at which point in time the code should appear.  
...
`r reveal("cars", reveal_type = "non_seq")`


---

## `reveal_type = 5` (or "entering the multiverse")

Another modality is to set reveal_type equal to a positive integer, indicating that you want the same code chunk to be displayed multiple times.  This makes the most sense in a setting where there is some randomization or random sampling and you want to see different realizations. Let's see this used on the user input code chunk "cars_multi", whose first step is to randomly sample rows from the data set cars with replacement.  
...
`r reveal("cars_multi", reveal_type = 3)`
...
`r reveal("cars", reveal_type = "user", display_type = "output")`
...
`r reveal("cars", reveal_type = "auto", display_type = "code")`
...
`r reveal("left_assign", reveal_type = "auto", display_type = "both", left_assign = TRUE)`
...
`r reveal("left_assign2", reveal_type = "auto", display_type = "both", left_assign = TRUE)`
