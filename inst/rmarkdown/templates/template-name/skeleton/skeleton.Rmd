...
parsed %>% 
  reveal_parsed(break_point = 3, highlight = 2:3) 
...
```{r}
# return_partial_side_by_side_code_output_chunks(chunk_name = "nobel")
```
...
# return_partial_code_or_output_chunks(chunk_name = "nobel")
```
...
count: false
My Title
```{r output_nobel_1, eval=TRUE, echo = FALSE, code=reveal_chunk('nobel', 1, 1, FALSE)}
```

---

```{r}
partially_knit_chunks(chunk_name = "nobel")
```
...
My Title
.column[.content[
```{r code_nobel_1, eval=FALSE, echo = TRUE, code=reveal_chunk('nobel', 1, 1, FALSE)}
```
]]
.column[.content[
```{r output_nobel_1, eval=TRUE, echo = FALSE, code=reveal_chunk('nobel', 1, 1, FALSE)}
...
My Title
.column[.content[
```{r code_nobel_2, eval=FALSE, echo = TRUE, code=reveal_chunk('nobel', 2, 2, FALSE)}
```
]]
.column[.content[
```{r output_nobel_2, eval=TRUE, echo = FALSE, code=reveal_chunk('nobel', 2, 2, FALSE)}
```
]]
---
...
*** apply_reveal("regular_assignment", reg_assignment = T)`
...
*** apply_reveal_nonsequential("non_sequential")`
